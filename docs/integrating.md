# Integrating LiquidityFloor

## From Solidity

```solidity
import {LiquidityFloorHook} from "liquidity-floor/src/hooks/LiquidityFloorHook.sol";
...
hook.configure(key, LiquidityFloorHook.Config({ /* see the README */ }));
...
const address = hookAddress("liquidity-floor", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "LiquidityFloor"
...
More at https://liquidity-floor.pages.dev.
