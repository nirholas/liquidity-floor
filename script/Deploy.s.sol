...
/**
 * @title DeployLiquidityFloor
 * @notice Deploys LiquidityFloor deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
...
contract DeployLiquidityFloor is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(LiquidityFloorHook).creationCode;
        ...
        console2.log("LiquidityFloor ", predicted);
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("LiquidityFloor")),
